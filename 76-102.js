...
const reverseCase = s => s.replace(/./g, char => char[`to${/[a-z]/.test(char) ? "Upper" : "Lower"}Case`]())

//78) return a random integer in given range
const createRange = (a, b) => Array.from({ length: b - a + 1 }, (_, i) => a + i);
...
const correctStream = (correct, test) => correct.map((v, i) => [, test[i]].indexOf(v))
...
const countNumberOfOccurrences = obj => [...new Set(Object.values(obj))].reduce((finalObj, key) => {
...
const longest7SegmentWord = arr => arr.filter(v => !/[kmvwx]/i.test(v)).sort((a, b) => b.length - a.length)[0]


console.log(longest7SegmentWord(["knighthood", "parental", "fridge", "clingfilm"]))


const findOdd = arr => [...new Set(arr)].map(v => [v, arr.filter(x => v === x).length]).find(([x, y]) => y % 2)[0]
...
const mapping = letters => letters.reduce((finalObj, key) => (finalObj[key] = key.toUpperCase(), finalObj), {})
console.log(mapping(["p", "s"]))

const getMissingLetters = str => Array.from({length: 26}, (_, i) => String.fromCharCode(97 + i)).join('').replace(new RegExp(`[${str}]`, 'gi'), '')
...
const convertBinary = str => [...str].map(char => [1, 0][+/[a-m]/i.test(char)]).join('')
...
const abcmath = (a, b, c) => {
  let total = a;
  Array(b).fill(0).forEach(_ => total += total);
  return total % c === 0
} 
//with while loop
...
*/
const strMatchBy2char = (s1, s2) => [...s1].reduce((a, c, i) => a + (c + s1[i + 1] === s2[i] + s2[i + 1] && i !== s1.length - 1), 0)
//85()
...
const carMaker = {}
const carLot = car => num => carMaker[car] ? (carMaker[car] += num) : (carMaker[car] = num)
...
//89)
const formatPhoneNumber = nums => nums.join``.replace(/(\d{3})(\d{3})(\d{4})/, `($1) $2-$3`)
...
const reducer = arr => arr.reduce((a, c) => a.concat(Array.isArray(c) ? reducer(c) : c), [])
const whereIsWaldo = arr => {
	const waldo = reducer(arr).find((v, i, thisarg) => thisarg.indexOf(v) === thisarg.lastIndexOf(v));
...
const funcSorts = arr => arr.sort((a, b) => testLengthRevised(a, 0) - testLengthRevised(b, 0))
/*
...
const fruitSalds = arr => arr.reduce((a, c) => [...a, c.slice(0, c.length / 2), c.slice(c.length / 2)], []).sort().join``
...
const fruitSald = arr => arr.flatMap(v => [v.slice(0, v.length / 2), v.slice(v.length / 2)]).sort().join``
...
const countOnesss = arr => arr.join``.split(/0+/).filter(num => num.length > 1).length
...
const chosenWine = wine => {
	try {
		return wine.sort(({price: priceA}, {price: priceB}) => priceA - priceB)[wine.length > 1 ? 1 : 0].name
	} catch(e) {
		return null
	}
}
...
const getFrequencies = arr => Object.fromEntries([...new Set(arr)].map(k => [k, arr.filter(v => v === k).length]))
