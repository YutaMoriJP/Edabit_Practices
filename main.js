...
const countNumber = arr => arr.reduce((a, c) => a + (Array.isArray(c) ? countNumber(c) : typeof c === 'number'), 0)
...
const hasHiddenFee = (p, t) => {
  return p.reduce((a, c) => a + +c.replace(/\D/g, ''), 0) <  +t.replace(/\D/g, '')
}
console.log(hasHiddenFee(["$2", "$4", "$1", "$8"], "$15"))

const flash = ([n1, op, n2]) => n2 === 0 ? undefined : {'x'(n1, n2){return n1 * n2}, '/'(n1, n2){return n1 / n2}, '+'(n1, n2){return n1 + n2}, '-'(n1, n2){return n1 - n2}}[op](n1, n2)
...
class Test {
  ...
  intro(x, y){return `My name is ${this.name} and I am ${this.age} years old. My goal is ${x} and to do ${y}`}
}

const t1 = new Test('Yuta', 25);
console.log(t1.intro('Die', 'Kill'));

console.log(Test.prototype.intro.call({name: 'Yuta', age: 25}, 'Die', 'Kill'))
...
const amazingEdabit = s => !s.includes('edabit') ? s.replace(/amazing/, word => `not ${word}`) : s
...
const smallerNum = (n1, n2) => [...n1].reduce(reducer, 0) > [...n2].reduce(reducer, 0) ? n2 : n1
...
const error = n => {
	const errorMessage = ['Check the fan', 'Emergency stop', 'Pump Error', 'c', 'Temperature Sensor Error'][n - 1];
...
const errors = n => ['Check the fan: e1', 'Emergency stop: e2', 'Pump Error: e3', 'c: e4', 'Temperature Sensor Error: e5'][n - 1] || 101

//Same answers as above, but instead of writing [n - 1] in the accessor statement
//we add an empty string

const errorss = n => ['', 'Check the fan: e1', 'Emergency stop: e2', 'Pump Error: e3', 'c: e4', 'Temperature Sensor Error: e5'][n] || 101
...
const ASTWOtoBit = mapMethod.call('hello, world', function(value) {return value.charCodeAt()})
...
const calculateArrowhead = arr => {
  const right = arr.filter(arrow => /\>/.test(arrow)).reduce((a, c) => a + c.length, 0)
  const left = arr.filter(arrow => /\</.test(arrow)).reduce((a, c) => a + c.length, 0)
  return (left > right ? '<' : '>').repeat(Math.max(right, left) - Math.min(right, left))
}
...
function middleEarth(arr) {
	return arr.some((name, i) => name === 'Frodo' ? arr[i - 1] === 'Sam' || arr[i + 1] === 'Sam' : name === 'Sam' ? arr[i - 1] === 'Frodo' || arr[i + 1] === 'Sam' : false)
}
...
function operation(a, b, op) {
	return {add(a,b){return a + b},subtract(a,b){return a-b},divide(a,b){return b==0?undefined:a/b},multiply(a,b){return a*b}}[op](a,b)
}
...
//using slice
const formatBigInts = (bigNubmer, decimal) => bigNubmer.toString().slice(0, -decimal) + '.' + bigNubmer.toString().slice(-decimal)
...
const signAll = (o, s) => {
  Object.entries(o).forEach(([key, nestedObj]) => {
    if(key === 'signature'){o[key] = s}
    Object.keys(nestedObj).forEach(nestedKey => {
      if(nestedKey === 'signature') {o[key][nestedKey] = s}
    })
  })
  return o
}
...
const isConfigurable = obj => Object.defineProperty(obj, 'nope', {
  value: 10,
  writable: true,
  enumerable: true,
...
const isExtensible = (o) => {
  ...
  keys.forEach(key => Object.defineProperty(o, key, {
    writable: false,
    enumerable: true,
    configurable: false
  }));
...
const whichIsLarger = (f, g) => f() > g() ? f.name : g() > f() ? g.name : 'neither';

console.log(whichIsLarger(function f(){return 10}, function g(){return 11}))
...
//28) join numbers:
const joinDigits = n => Array.from({length:n}, (_,i) => (i + 1 + '').split('').join('-')).join('-')
...
const makeGrlexx = arr => arr.sort((a, b) => a.length - b.lengt || a.localeCompare(b))
...
function move(word) {
	return [...word].map(char => String.fromCharCode(char.charCodeAt() + 1)).join('')
}
...
const duplicateCount = str => [...str].filter((v,i) => str.indexOf(v) !== i).filter((v, i, thisArr) => thisArr.indexOf(v) === i).length
...
*/
const oldest = p => Object.entries(p).sort(([keyA, valueA], [keyB, valueB]) => valueB - valueA)[0][0]
console.log(oldest({
  Emma: 71,
  Jack: 45,
  Amy: 15,
...
const convertToNumber = obj => Object.fromEntries(Object.entries(obj).map(([key, value]) => [key, +value]))
...
const singleOccurrence = str => [...str.toUpperCase()].find((v, i, thisarg) => thisarg.indexOf(v) === thisarg.lastIndexOf(v)) || ''
...
const spaceMessage = s => !s.includes('[') ? s : spaceMessage(s.replace(/\[[\w]+\]/g, v => {
  v = v.slice(1, -1);
  return v.match(/[a-z]+/i).join('').repeat(v.replace(/[a-z]+/gi, ''))
}))
...
//39)
//animals(2, 3, 5)
const animals = (c, cc, p, cow = c * 2, crow = cc * 4, pig = p * 4) => cow + crow + pig
console.log(animals(2, 3, 5))


//40 After N Months...

const afterNMonths = (y, m) => y + Math.floor(m / 12) || (y ? 'month missing' : 'year missing')
console.log(afterNMonths(2222, 24))

//41 Return most optimized name

const mostOptimized = arr => arr.reduce((a, c) => c.optimized > (a.optimized || 0) ? c : a, {})

console.log(mostOptimized([{name: 'A', optimized: 5}, {name: 'B', optimized: 7}, {name: 'C', optimized: 1}, {name: 'D', optimized: 10}]))
...
function chatroomStatus(users) {
	return !users.length ? 'no one online' : users.length === 1 ? `${users[0]} online` : users.length === 2 ? `${users[0]} and ${users[1]} online` : `${users[0]}, ${users[1]}, and ${users.length - 2} more online`
}
...
const memoize = (fn) => {
  ...
  return (arg) => {
    ...
    if(cache[arg]) {
      console.log(`Fetching result from the cache object with the proeprty ${arg}`)
      return cache[arg]
    }
    console.log(`Input is new so calculating the result of fn(arg) and store it in result and then cache object`)
    const result = fn(arg)
    cache[arg] = result
    return result
  }
...
const lonelyInteger = nums => nums[nums.map(n => n * (n > 0 ? -1 : 1)).findIndex((n, _, thisarg) => thisarg.indexOf(n) === thisarg.lastIndexOf(n))]

const lonelyIntegers = nums => nums.find(n => nums.filter(v => v === (n * -1)).length === 0)
...
const promise = (n) => new Promise((resolve, reject) => {
  n > 0 ? (setTimeout(() => resolve('number is larger than 0'), 1000)) : reject('invalid n')
})
...
countDwarves = function() {return 'hi'}
...
const ageDifference = ages => {
	const [s1, s2] = [...ages].sort((a, b) => b - a), diff = s1 - s2;
...
const sortWord = w => [...w].sort((a, b) => /[A-Z]/.test(`${a}${b}`) ? 0 : a.localeCompare(b)).join('')

console.log(sortWord("Unpredictable"))
//sortWord("Unpredictable") ➞ "Uabcdeeilnprt"

//48)
const filterStateNames = (arr, type) => arr.filter(v => type === 'abb' && v.length === 2 || type === 'full' && v.length > 2)
...
const reorderDigits = (a, d) => a.reduce((a, c) => a.concat(+[...`${c}`].sort((a, b) => d === 'asc' ? +a - +b : +b - +a).join('')), [])
/*
...
const search = (a, item) => !a.includes(item) ? -1 : a.slice(-1)[0] === item ? a.length - 1 : search(a.slice(0, -1), item)
//search([1, 2, 3, 4], 3) ➞ 2
...
const strictEqual = (a, b) => Number.isNaN(a || b) ? false : a === -0 && b === 0 || b === -0 && a === 0 ? true : Object.is(a, b)
...
const scaleTip = arr => {
  ...
  return leftVal > rightVal ? 'left' : rightVal > leftVal ? 'right' : 'balanced'
}
